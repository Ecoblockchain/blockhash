...
void translate_blocks_to_bits(int *blocks, int nblocks, int pixels_per_block)
{
    ...
    int bandsize, i, j, v, s;
    float m;
    int *tmp;

    tmp = calloc(nblocks, sizeof(int));
    s = sqrt(nblocks);
    for (i = 0; i < s; i++) {
      for (j = 0; j < s; j++) {
       if (j % 2 == 0) {
          memcpy(&tmp[i*s+j], &blocks[i*s+j], sizeof(int));
       } else {
          memcpy(&tmp[i*s+j], &blocks[(s-(i+1))*s+j], sizeof(int));
       }
      }
    }
    ...
    for (i = 0; i < 2; i++) {
        m = median(&tmp[i * bandsize], bandsize);
...
void translate_blocks_to_bitsf(float *blocks, int *result, int nblocks, int pixels_per_block)
{
    ...
    s = sqrt(nblocks);
    tmp = calloc(nblocks, sizeof(float));
    for (i = 0; i < s; i++) {
      for (j = 0; j < s; j++) {
       if (j % 2 == 0) {
          memcpy(&tmp[i*s+j], &blocks[i*s+j], sizeof(float));
       } else {
          memcpy(&tmp[i*s+j], &blocks[(s-(i+1))*s+j], sizeof(float));
       }
      }
    }
    ...
    for (i = 0; i < 2; i++) {
        m = medianf(&tmp[i * bandsize], bandsize);
        for (j = i * bandsize; j < (i + 1) * bandsize; j++) {
            v = tmp[j];
